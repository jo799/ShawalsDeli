...
// single-tender cash or card sale rung up while disconnected. It does not
// cover M-Pesa (needs a live call to Safaricom — there's no way to queue
// that), split bills, or loyalty point redemption (needs a live, current
// points balance to avoid overspending points that may have moved in the
// meantime) — those stay blocked while offline rather than pretending to
// support them.
